...
func configHandler(configMsgChan chan *configmodels.ConfigMessage, configReceived chan bool, oneDeleter MongoOneDeleter, manyGetter MongoManyGetter, poster MongoPoster, oneGetter MongoOneGetter) {
	// Start Goroutine which will listens for subscriber config updates	and update the mongoDB. Only for 5G.
	subsUpdateChan := make(chan *Update5GSubscriberMsg, 10)
	if factory.WebUIConfig.Configuration.Mode5G == true {
		go Config5GUpdateHandle(subsUpdateChan, oneDeleter, manyGetter, poster, oneGetter)
	}
...
func Config5GUpdateHandle(confChan chan *Update5GSubscriberMsg, oneDeleter MongoOneDeleter, manyGetter MongoManyGetter, poster MongoPoster, oneGetter MongoOneGetter) {
	for confData := range confChan {
		...
		case configmodels.Sub_data:
			...
			if confData.Msg.MsgMethod != configmodels.Delete_op {
				...
				poster.RestfulAPIPost(authSubsDataColl, filter, authDataBsonA)
			} else {
				logger.WebUILog.Debugln("Delete AuthenticationSubscription", imsi)
				filter := bson.M{"ueId": "imsi-" + imsi}
				oneDeleter.RestfulAPIDeleteOne(authSubsDataColl, filter)
