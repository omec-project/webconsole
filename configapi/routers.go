...
func AddService(engine *gin.Engine, oneGetter server.MongoOneGetter, manyGetter server.MongoManyGetter) *gin.RouterGroup {
	group := engine.Group("/config/v1")
	var routes = GetRoutes(oneGetter, manyGetter)
...
func GetRoutes(oneGetter server.MongoOneGetter, manyGetter server.MongoManyGetter) Routes {
	return Routes{
		{
			"Index",
			http.MethodGet,
...

		{
			...
			GetDeviceGroupByName(oneGetter),
		},
...

		{
			...
			GetNetworkSliceByName(oneGetter),
		},
