...
const (
	gnbByNamePath = "/config/v1/inventory/gnb/gnb1"
	upfByFQDNPath = "/config/v1/inventory/upf/upf1.my-domain.com"
	upfByNamePath = "/config/v1/inventory/upf/upf1"

	gnbCreateBody   = `{"name": "gnb1", "tac": 123}`
	gnbTacOnlyBody  = `{"tac": 123}`
	upfCreateBody   = `{"hostname": "upf1.my-domain.com", "port": "123"}`
	upfPortOnlyBody = `{"port": "123"}`
)
...
func TestGetInventory_DBError(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedBody: map[string]string{errorKey: "failed to retrieve gNBs"},
		},
		{
			name:         "UpfDBError",
			route:        upfInventoryPath,
			...
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to retrieve UPFs"},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestGnbPostHandler(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedBody: map[string]string{errorKey: "gNB already exists"},
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: errMsgInvalidJSONFormat},
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid gNB TAC '0'. TAC must be an integer within the range [1, 16777215]"},
		},
		{
			...
			inputData:    gnbCreateBody,
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to create gNB"},
		},
		{
			...
			inputData:    `{"tac": 12}`,
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid gNB name ''. Name needs to match the following regular expression: " + NAME_PATTERN},
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid gNB name 'gn!b1'. Name needs to match the following regular expression: " + NAME_PATTERN},
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid gNB name '" + genLongString(257) + "'. Name needs to match the following regular expression: " + NAME_PATTERN},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestGnbPutHandler(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			inputData:    gnbTacOnlyBody,
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to PUT gNB"},
		},
		{
...
func TestUpfPostHandler(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			inputData:    upfCreateBody,
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "UPF already exists"},
		},
		{
		...
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid UPF port ''. Port must be a numeric string within the range [0, 65535]"},
		},
		{
			name:         "DB POST operation fails expects failure",
			route:        upfInventoryPath,
			...
			inputData:    upfCreateBody,
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to create UPF"},
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid UPF port 'a'. Port must be a numeric string within the range [0, 65535]"},
		},
		{
			...
			inputData:    `{"port": "a"}`,
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid UPF hostname ''. Hostname needs to represent a valid FQDN"},
		},
		{
			...
			expectedCode: http.StatusBadRequest,
			expectedBody: map[string]string{errorKey: "invalid UPF hostname 'upf1'. Hostname needs to represent a valid FQDN"},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestUpfPutHandler(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			inputData:    upfPortOnlyBody,
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to PUT UPF"},
		},
		{
			name:         "Port cannot be converted to int expects failure",
			route:        upfByFQDNPath,
...
func TestInventoryDeleteHandlers(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			dbAdapter:    &GnbMockDBClient{err: fmt.Errorf("mock error")},
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to delete gNB"},
		},
		{
		...
		},
		{
			...
			dbAdapter:    &UpfMockDBClient{err: fmt.Errorf("mock error")},
			expectedCode: http.StatusInternalServerError,
			expectedBody: map[string]string{errorKey: "failed to delete UPF"},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
